...
- Repository는 DB에서 반환된 원시 row 객체(snake_case)를 그대로 반환한다. snake_case → camelCase 변환은 `src/utils/format.js`의 `formatUser`, `formatCategory`, `formatTodo` 함수가 담당하며, Service 레이어에서 호출한다.
...
| DB 쿼리 결과 필드 | snake_case → camelCase 변환 (`src/utils/format.js`) | `user_id` → `userId` |
...
-- 애플리케이션 코드: camelCase (src/utils/format.js → Service 레이어에서 변환)
...
```
백엔드:
  tests/services/todo.service.test.js             (서비스 단위 테스트)
  tests/services/category.service.test.js
  tests/services/user.service.test.js
  tests/repositories/todo.repository.test.js      (Repository Mock 테스트)
  tests/repositories/category.repository.test.js
  tests/repositories/user.repository.test.js
  tests/routes/todo.routes.test.js                (라우트 통합 테스트)
  tests/routes/category.routes.test.js
  tests/routes/auth.routes.test.js
  tests/routes/user.routes.test.js
  tests/middlewares/authenticate.test.js
  tests/middlewares/validate.test.js
  tests/utils/jwt.test.js
  tests/utils/password.test.js
  tests/config/env.test.js
  tests/db/pool.test.js
  tests/types/errors.test.js
  tests/app.test.js
...
- 백엔드 (JavaScript): `eslint:recommended` 기반에 Node.js 환경 설정 추가.
...
실제 운영/개발에 사용하는 환경 변수 파일. `.gitignore`에 반드시 포함. `.env.example`을 복사하여 실제 값으로 채워 사용한다.
...
POSTGRES_CONNECTION_STRING=postgresql://<user>:<password>@localhost:5432/todolist_app
...
# Derived connection string (postgresql-mcp 및 앱 설정에서 사용)
POSTGRES_CONNECTION_STRING=postgresql://${DB_USER}:${DB_PASSWORD}@${DB_HOST}:${DB_PORT}/${DB_NAME}

# Backend
NODE_ENV=development
PORT=3000
JWT_SECRET=change_me_to_a_random_32_character_string_minimum  # 최소 32자 이상
JWT_EXPIRES_IN=24h
BCRYPT_SALT_ROUNDS=10
CORS_ORIGIN=http://localhost:5173
...
backend/
├── src/
│   ├── app.js                      # Express 앱 생성, 미들웨어 등록, 라우터 연결, HTTP 서버 시작
│   │
│   ├── config/                     # 설정 모음
...
│   │   ├── authenticate.js         # JWT 검증 미들웨어 (req.userId 설정)
│   │   ├── validate.js             # 입력 유효성 검증 미들웨어 팩토리
│   │   └── error-handler.js        # 글로벌 에러 핸들러 (AppError, PostgreSQL 22P02 처리)
│   │
...
│   │
│   ├── repositories/               # Repository: pg를 이용한 DB 쿼리 실행 (raw row 반환)
│   │   ├── user.repository.js      # users 테이블 쿼리 (findByEmail, findById, create, update)
│   │   ├── todo.repository.js      # todos 테이블 쿼리 (CRUD, 동적 필터 조회)
│   │   └── category.repository.js  # categories 테이블 쿼리 (CRUD, 중복 검사, 할일 수 집계)
│   │
│   ├── types/                      # 공통 타입/상수 정의
│   │   └── errors.js               # 커스텀 에러 클래스 (AppError, NotFoundError, ForbiddenError, ConflictError)
│   │
│   └── utils/                      # 공통 유틸리티 함수
│       ├── format.js               # DB row → camelCase API 응답 변환 (formatUser, formatCategory, formatTodo)
│       ├── password.js             # bcrypt 해시/비교 함수
│       └── jwt.js                  # JWT 생성/검증 함수
│
├── tests/                          # 단위 테스트 (Jest + pg Pool Mock)
│   ├── app.test.js
│   ├── config/
│   │   └── env.test.js
│   ├── db/
...
> **참고:**
> - 스키마 파일은 `database/schema.sql` (프로젝트 루트 기준)에 위치한다.
> - Swagger 명세는 `swagger/swagger.json`에 위치하며, 서버 구동 후 `/api-docs`에서 Swagger UI를 확인할 수 있다.
> - 루트 `.env` / `.env.example`은 postgresql-mcp 개발 도구 전용(`POSTGRES_CONNECTION_STRING`)만 포함한다. 백엔드 환경 변수는 `backend/.env` / `backend/.env.example`에서 관리한다.
...
│   │   │   └── TodoCalendar.tsx    # 월별 달력 뷰 (날짜 클릭으로 할일 등록 이동)
│   │   ├── categories/
│   │   │   ├── CategoryItem.tsx    # 카테고리 항목 (수정/삭제 UI 포함)
│   │   │   └── CategoryForm.tsx    # 카테고리 추가 폼
│   │   └── layout/
│   │       └── Header.tsx          # 좌측 사이드바 (로그인 사용자명, 네비게이션, 로그아웃)
│   │
...
│   │   └── apiError.ts             # Axios 에러에서 ApiError 추출 유틸
│   │
│   └── styles/                     # 전역 스타일
...
├── frontend/                   # 프론트엔드 (React 19 + TypeScript) — 개발 예정
├── database/
│   └── schema.sql              # PostgreSQL 스키마 + 기본 카테고리 시드 데이터
├── swagger/
│   └── swagger.json            # OpenAPI 3.0 API 명세 (백엔드 /api-docs에서 서빙)
├── docs/                       # 프로젝트 문서
...
│   ├── 6-erd.md
│   └── 7-execution-plan.md
├── .env                        # postgresql-mcp 전용 (POSTGRES_CONNECTION_STRING, Git 제외)
├── .env.example                # postgresql-mcp 전용 템플릿 (Git 포함)
